...
function flickr_decrypt_key( $cipher ) {
    ...
    return openssl_decrypt( base64_decode( $parts[1] ), $algo, 'hacklabr_flickr', 0, base64_decode( $parts[0] ), base64_decode( $parts[2] ) );
}
...
function flickr_encrypt_key( $api_key ) {
    ...
    $cipher = openssl_encrypt( $api_key, $algo, 'hacklabr_flickr', 0, $iv, $tag );
...
function flickr_get_albums( $api_key, $user_id = '', $per_page = 10, $page = 1 ) {
    $cache_key = 'hacklabr_flickr_albums_' . $user_id . '_' . $per_page . '_' . $page;
...
function flickr_get_photos( $api_key, $flickr_by_type = 'user', $data_id = '', $per_page = 10, $page = 1 ) {
    $cache_key = 'hacklabr_flickr_photos_' . $flickr_by_type . '_' . $data_id . '_' . $per_page . '_' . $page;
    $cached_data = get_transient( $cache_key );
