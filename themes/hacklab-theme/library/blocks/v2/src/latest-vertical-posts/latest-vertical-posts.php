...
function latest_vertical_posts_callback( $attributes ) {
    ...
    $block_model     = ( isset( $attributes['blockModel'] ) && ! empty( $attributes['blockModel'] ) ) ? esc_attr( $attributes['blockModel'] ) : 'post';
    ...
    if ( $block_model == 'post' || $block_model == 'numbered' || $block_model == 'most-read' ){
    ...
    if ( $block_model == 'collection' ) {
        // Flickr
        require_once get_stylesheet_directory() . '/library/blocks/v2/includes/flickr.php';
        ...
        if ( ! $api_key || ! $data_id ) {
            if ( is_admin() || defined( 'REST_REQUEST' ) && REST_REQUEST ) {
                return '<h2>' . __( 'Check the API Key, user or album ID', 'hacklabr' ) . '</h2>';
            }
    ...
    if ( $block_model == 'videos' ) {
        ...
        if ( ! $api_key || ! $playlist_id ) {
            if ( is_admin() || defined( 'REST_REQUEST' ) && REST_REQUEST ) {
                return '<h2>' . __( 'Check the API Key or playlist ID', 'hacklabr' ) . '</h2>';
            }
            return;
        }
    ...
    if ( ! empty( $heading ) ) {
     ...
     if ( $block_model == 'collection' ) {
        ...
        foreach( $has_content['data'] as $photo ) :
            ...
            get_template_part( 'library/blocks/v2/src/latest-vertical-posts/template-parts/post', $block_model, [ 'photo' => $photo ] );
    ...
    if ( $block_model == 'columnists' ) {
        ...
        foreach ( $has_content as $author ) :
            ...
            get_template_part( 'library/blocks/v2/src/latest-vertical-posts/template-parts/columnist', '', ['author' => $author, 'attributes' => $attributes] );
    ...
    if ( $block_model == 'videos' ){
        ...
        foreach( $has_content as $video ) :
            ...
            get_template_part( 'library/blocks/v2/src/latest-vertical-posts/template-parts/post', $block_model, ['video' => $video, 'attributes' => $attributes] );
    ...
    if ( $block_model == 'post' || $block_model == 'numbered' || $block_model == 'most-read' ) {
        ...
        if ( $has_content->have_posts() ) :
            ...
            while ( $has_content->have_posts() ) :
                ...
                get_template_part( 'library/blocks/v2/src/latest-vertical-posts/template-parts/post', '', ['post' => $post, 'attributes' => $attributes] );
