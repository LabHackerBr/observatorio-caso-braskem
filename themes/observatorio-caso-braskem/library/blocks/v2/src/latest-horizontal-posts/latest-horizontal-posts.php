...
function latest_horizontal_posts_callback( $attributes ) {
    ...
    if ( $block_model == 'videos' ) {
        // Vídeos
        require_once  get_stylesheet_directory() . '/library/blocks/v2/includes/videos.php';
        ...
        if ( ! $api_key ) {
            if ( is_admin() || defined( 'REST_REQUEST' ) && REST_REQUEST ) {
                return '<h2>' . __( 'Check the API Key', 'hacklabr' ) . '</h2>';
            }
            return;
        }
            ...
            echo '<div class="latest-horizontal-posts-block__slides">';
                ...
                if ( $block_model == 'albums' ) {
                    ...
                    foreach( $has_content['data'] as $album ) :
                        get_template_part( 'library/blocks/v2/src/latest-horizontal-posts/template-parts/post', $block_model, [ 'album' => $album ] );
                    endforeach;
                }
                ...
                if ( $block_model == 'collection' ) {
                    ...
                    foreach( $has_content['data'] as $photo ) :
                        get_template_part( 'library/blocks/v2/src/latest-horizontal-posts/template-parts/post', $block_model, [ 'photo' => $photo ] );
                    endforeach;
                }
                ...
                if ( $block_model == 'columnists' ) {
                    ...
                    foreach ( $has_content as $author ) {
                        echo "<div class='slide'>";
                            get_template_part( 'library/blocks/v2/src/latest-horizontal-posts/template-parts/co', 'author', ['author' => $author] );
                        echo "</div>";
                    }
                }
                ...
                if ( $block_model == 'most-read' ) {
                    ...
                    while ( $has_content->have_posts() ) :
                        ...
                        echo "<div class='slide'>";
                            get_template_part( 'library/blocks/v2/src/latest-horizontal-posts/template-parts/post', $block_model, ['post' => $post, 'attributes' => $attributes] );
                        echo "</div>";
                    endwhile;
                ...
                if ( $block_model == 'videos' ) {
                    ...
                    foreach( $has_content as $video ) :
                        get_template_part( 'library/blocks/v2/src/latest-horizontal-posts/template-parts/post', $block_model, ['video' => $video, 'attributes' => $attributes] );
                    endforeach;
                }
