...
function stories_mosaic_callback( $attributes ) {
    ...
    if ( ! empty( $heading ) ) {
     ...
     if ( $block_model == 'collection' ) {
        ...
        foreach( $has_content['data'] as $photo ) :
            ...
            get_template_part( 'library/blocks/v2/src/stories-mosaic/template-parts/post', $block_model, [ 'photo' => $photo ] );
    ...
    if ( $block_model == 'post' || $block_model == 'numbered' || $block_model == 'most-read' ) {
        ...
        if ( $has_content->have_posts() ) :
            ...
            while ( $has_content->have_posts() ) :
                ...
                get_template_part( 'library/blocks/v2/src/stories-mosaic/template-parts/post', '', ['post' => $post, 'attributes' => $attributes] );
